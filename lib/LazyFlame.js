...
export default class extends Component {
  constructor(props) {
    ...
    this.state.set = vars => {
      for (const varName in vars) {
        ...
        this.setState(state => {
          log(`[LazyFlame] state.${varName} ${JSON.stringify({
          ...
          if (typeof val !== 'object') return {
            ...
            [varName]: Object.assign({}, state[varName], val)
          };
        });
      }
    };
  ...
  download(varName, tmpl, impl) {
    return snapshot => {
      ...
      if (!tmpl && !impl) {
        log(`[LazyFlame] download ${JSON.stringify({
          [varName]: val
        }, null, 2)}`);
        this.setState({
          [varName]: val
      ...
      this.setState(state => {
        ...
        ptr[v] = val;
        log(`[LazyFlame] download (recursive) ${JSON.stringify({
          [varName]: state[varName]
        }, null, 2)}`);
        return state;
      });
  ...
  render() {
    ...
    for (const prop in this.tmpl) {
      ...
      log(`[LazyFlame] new paths ${prop}@${JSON.stringify(Object.keys(newPaths), null, 2)}`); // add in
